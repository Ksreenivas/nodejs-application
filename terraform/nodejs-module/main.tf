...
resource "aws_instance" "nodejs_app" {
  ...
  security_groups = [aws_security_group.nodejs_app.name]
...
# A User Data script that will run when the EC2 instance boots up and start the Nodejs Server inside docker container
